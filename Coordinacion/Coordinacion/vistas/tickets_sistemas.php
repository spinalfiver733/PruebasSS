...
?>
    <!-- Inicio Contenido PHP-->
       ...
       <!--usuarios conectados-->              
            <?php  
                    $sql_fetch_todos = "SELECT * FROM tickets WHERE TIPO = 'SISTEMAS' GROUP BY id DESC";
...
<div class="table-responsive">  
                            ...
                            <?php 
                              if ($row['estado'] == 'sin asignar'){
                                  ...
                                  <td style="background-color:red;border-radius:50%;">  
                                      <a href="asignar_ticket_sistemas.php?id=<?php echo $row['id'] ;?>" style="color:white;text-align:center;">SIN ASIGNAR</a></td>
                                    <?php
                              }
       ...
       <!--usuarios conectados-->              
        ...
        <h2 STYLE="COLOR:WHITE;"><STRONG>USUARIOS CONECTADOS : SISTEMAS</STRONG></h2>
            <?php  
                    $sql_fetch_todos = "SELECT * FROM usuarios where conectado = 1 AND direccion = 'tecnico_sistemas'";
                    $query = mysqli_query($conexion, $sql_fetch_todos);
            ?>
        <table border ="1" class="table - table-striped">
            <tbody>

...
?>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const tipoTicket = document.querySelector('select[name="tipo"]');
                const problemaSelect = document.querySelector('select[name="problema"]');
                // Guardamos todos los problemas originales
                const problemas = Array.from(problemaSelect.options).map(option => ({
                    valor: option.value,
                    texto: option.text,
                    tipo: option.getAttribute('data-tipo')
                }));
                // Función para filtrar problemas según el tipo de ticket
                function filtrarProblemas(tipo) {
                    // Limpiamos el select de problemas
                    problemaSelect.innerHTML = '';
                    // Filtramos los problemas según el tipo seleccionado
                    let problemasFiltrados;
                    switch (tipo) {
                        case 'SISTEMAS':
                            problemasFiltrados = problemas.filter(problema =>
                                problema.tipo === 'SISTEMAS'
                            );
                            break;
                        case 'SOPORTE':
                            problemasFiltrados = problemas.filter(problema =>
                                problema.tipo === 'SOPORTE TECNICO'
                            );
                            break;
                        default:
                            // Para CTI o cualquier otro caso, mostramos todos los problemas
                            problemasFiltrados = problemas;
                            break;
                    }
                    // Agregamos las opciones filtradas
                    problemasFiltrados.forEach(problema => {
                        const option = new Option(problema.texto, problema.valor);
                        option.setAttribute('data-tipo', problema.tipo);
                        problemaSelect.add(option);
                    });
                }
                // Evento change del tipo de ticket
                tipoTicket.addEventListener('change', function() {
                    filtrarProblemas(this.value);
                });
                // Filtrar inicialmente - como CTI está seleccionado por defecto,
                // esto mostrará todos los problemas al cargar
                filtrarProblemas(tipoTicket.value);
            });
    ...
    // Formatear para asegurar dos dígitos
    const horaFormateada = hora.toString().padStart(2, '0');
    const minutoFormateado = minuto.toString().padStart(2, '0');
    const segundoFormateado = segundo.toString().padStart(2, '0');

    // Juntar todo en el formato deseado
    const horaCompleta = `${horaFormateada}:${minutoFormateado}:${segundoFormateado}`;
    
    // Actualizar el campo
    document.forms.form_reloj.reloj.value = horaCompleta;
                                ...
                                <div class="small-box bg-aqua">
                                    ...
                                    <div class="inner">
                                        ...
                                        <form action="cargar_ticket.php" method="POST" enctype="multipart/form-data">
                                            ...
                                            <table class="table table-striped">
                                                <tr>
                                                    <td style="float:center;">
                                                        <h2 style="color:#9F2241;"><strong>Tipo de ticket: </strong></h2>
                                                    </td>
                                                    <td style="float:left;">
                                                ...
                                                <tr>
                                                    <td style="float:center;">
                                                        <h2 style="color:#9F2241;"><strong> Problema a resolver: </strong></h2>
                                                    </td>
                                                    <td style="float:left;">
                                                        <select name="problema" style="font-size:1.5em;">
                                                            <?php
                                                            $sql_fetch_todos_problemas = "SELECT * FROM catalogo_problemas ORDER BY concepto ASC";
                                                            $query_problemas = mysqli_query($conexion, $sql_fetch_todos_problemas);
                                                            foreach ($query_problemas as $opciones_problemas):
                                                            ?>
                                                                <option value="<?php echo $opciones_problemas['concepto'] ?>"
                                                                    data-tipo="<?php echo $opciones_problemas['tipo_ticket'] ?>"
                                                                    style="color:black;">
                                                                    <?php echo $opciones_problemas['concepto'] ?>
                                                                </option>
                                                            <?php endforeach ?>
                                                        </select>
                                                    </td>
                                                ...
                                                <tr>
                                                    <td style="float:center;">
                                                        <h2 style="color:#9F2241;"><strong> Área en la que se presenta el problema: </strong></h2>
                                                    </td>
                                                    <td style="float:left;">
                                                ...
                                                <tr>
                                                    <td style="float:center;">
                                                        <h2 style="color:#9F2241;"><strong> Dirigirse a: </strong></h2>
                                                    </td>
                                                    <td style="float:left;">
                                                ...
                                                <tr>
                                                    <td style="float:center;">
                                                        <h2 style="color:#9F2241;"><strong> Prioridad: </strong></h2>
                                                    </td>
                                                    <td style="float:left;">
                                            ...
                                            <button type="submit" style="width: 100%;height: 100px;"><strong>
                                                    <h1 style="color:white;">
                                                        <li class="fa fa-ticket"></li><strong> GENERAR TICKET</strong>
                                                    </h1>
                                                </strong></button>
                                        </form>
                                    </div>
